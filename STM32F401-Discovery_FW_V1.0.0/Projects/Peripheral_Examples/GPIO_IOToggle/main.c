...
int HR_count=0;
int HR_Value=60;
int ECG_Rawdata[Rawdata_N]={0};
unsigned int ADC_Value[5]={0,0,0,0,0};
int count_t;
static long int LT_count=0;
int QRS_result;
int TempCount=0;
...
void BT_GPIO_Init(void){
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3 ;   //Pin0: Power   Pin1:RST   Pin2:Pairing   Pin3:SW_BTN   Pin4:Status
...
void BT_Start(void){
	...
	BT_Power(0);        //Give Power 'Low' as open，0 as low
	Delay(1000);        //Don't start component simutaneously
...
void SDCard_Write(void){
	...
	br = f_puts("1234567890", &file) ;  //Write "1234567890" into string
...
int getsample(long int tt,int *inputdata){
	if(tt<0)
		tt=0;
	return inputdata[tt&(Rawdata_N-1)];
...
/*================================================================================*/
 ...
 int main(void){	
	...
	TM_ADC_Init(ADC1,ADC_Channel_3); //Enable ADC1_Channel_3,ECG
	TM_ADC_Init(ADC1,ADC_Channel_12); //Enable ADC1_Channel_12,Temperature
	TIM_Config(525 - 1 , 1250 - 1); //128Hz,Clock=84MHz, (84000000/period)/prescaler=Freq
...
void TIM3_IRQHandler(){
	...
	if(TIM_GetITStatus(TIM3 , TIM_IT_Update) == SET) //確認是否有執行中斷
	{
		...
		{
			TM_USART_Putc(USART2,0x00);
			TM_USART_Putc(USART2,0x00);
			
			if(TM_USB_VCP_GetStatus()==TM_USB_VCP_CONNECTED){
				TM_USB_VCP_Putc(0xA5);	//Header
				TM_USB_VCP_Putc(0xA5);	//Header
				TM_USB_VCP_Putc(0x00);	//Gsensor_X,Low,Graph1
				TM_USB_VCP_Putc(0x00);	//Gsensor_X,High,Graph1
				TM_USB_VCP_Putc(0x00);	//ECG,Low,Graph2
				TM_USB_VCP_Putc(0x00);	//ECG,High,Graph2
				TM_USB_VCP_Putc(0x00);	//Gsensor_Y,Low,Graph3
				TM_USB_VCP_Putc(0x00);	//Gsensor_Y,High,Graph3
				TM_USB_VCP_Putc(0x00);	//Gsensor_Z,Low,Graph4
				TM_USB_VCP_Putc(0x00);	//Gsensor_Z,High,Graph4
				TM_USB_VCP_Putc(0x00);	//Temprature,Low
				TM_USB_VCP_Putc(0x00);	//Temprature,High
				TM_USB_VCP_Putc(0x00);	//Reddot on ECG
				TM_USB_VCP_Putc(0x00);	//Reddot on ECG
			}
		}
		else
		{
			Gsensor_readValue();
			ECGData = TM_ADC_Read(ADC1,ADC_Channel_3); //設定Data的值為ADC1的Channel3轉換出來的值
			if(TempCount==0)
			{
				TempData = TM_ADC_Read(ADC1,ADC_Channel_12);
				TempCount = 128;
			...
			ECGLow = ECGData&0x00ff;   //Low值取Data的低8bits
			ECGHigh = (ECGData>>8)&0x00ff; //High值先將Data Shift 8bits後再取值，不過因為ADC取12bits且低8bits已經使用過，所以High值只有12-8=4bits
			TempLow = TempData&0x00ff;
			TempHigh = (TempData>>8)&0x00ff;
			...
			ECG_Rawdata[LT_count&(Rawdata_N-1)]=ECGData-2048;
			QRS_result=qrs_detect(LT_count,ECG_Rawdata);
			...
			if(TM_USB_VCP_GetStatus()==TM_USB_VCP_CONNECTED){
				...
				TM_USB_VCP_Putc((ECG_Rawdata[(LT_count)&(Rawdata_N-1)]+2048)&0xFF);	//ECG,Low,Graph2
				TM_USB_VCP_Putc(((ECG_Rawdata[(LT_count)&(Rawdata_N-1)]+2048)>>8)&0xFF);	//ECG,High,Graph2
				...
				TM_USB_VCP_Putc((unsigned char)QRS_result);	//Reddot on ECG
				TM_USB_VCP_Putc((unsigned char)HR_Value);	//Reddot on ECG
			}
